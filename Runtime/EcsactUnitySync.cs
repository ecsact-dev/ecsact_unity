...
using TypeComponentIdsMap =
	System.Collections.Generic
		.Dictionary<System.Type, System.Collections.Generic.SortedSet<System.Int32>>;

using ComponentIdBehavioursMap =
	System.Collections.Generic
		.Dictionary<System.Int32, System.Collections.Generic.HashSet<System.Type>>;
...
public interface IOnInitComponent<T>
	where          T : Ecsact.Component {
  void OnInitComponent(in T component);
}

public interface IOnUpdateComponent<T>
	where          T : Ecsact.Component {
  void OnUpdateComponent(in T component);
}

public interface IOnRemoveComponent<T>
	where          T : Ecsact.Component {
...
public static class UnitySyncMonoBehaviours {
		...
		foreach(var i in monoBehaviour.GetType().GetInterfaces()) {
			...
			if(genericTypeDef == typeof(IOnInitComponent<>)) {
				...
				if(targetComponentType.Equals(componentType)) {
					i.GetMethod("OnInitComponent")
						.Invoke(monoBehaviour, new object[] { component });
				}
			}
		}
	}
	...
	public static IEnumerable<Type> GetTypes(ComponentIdsList componentIds) {
		var compIds =
			new ComponentIdsList(knownComponentIds.Intersect(componentIds));
	...
	public static bool HasInterfaces(Type type) {
	...
	public static IEnumerable<Int32> GetInitComponentIds(Type type) {
	...
	public static IEnumerable<Int32> GetRemoveComponentIds(Type type) {
		if(onInitComponentsMap.TryGetValue(type, out var compIds)) {
			foreach(var compId in compIds) {
				yield return compId;
			}
	...
	public static IEnumerable<Int32> GetRequiredComponentIds(Type type) {
	...
	public static bool RegisterMonoBehaviourType(Type type) {
		if(RegisterMonoBehaviourInterfaces(type)) {
			EndMonoBehaviourRegistration(type);
			return true;
		}
	...
	private static bool RegisterMonoBehaviourInterfaces(Type type) {
	...
	private static void BeginMonoBehaviourRegistration(Type monoBehaviourType) {
	...
	private static void EndMonoBehaviourRegistration(Type monoBehaviourType) {
	}
	...
	private static void AddKnownComponentId(Int32 componentId) {
	...
	private static HashSet<Type> GetRequiredBehaviours(Int32 componentId) {
	...
	private static HashSet<Type> GetOnInitBehaviours(Int32 componentId) {
		HashSet<Type> behaviours;
	...
	private static HashSet<Type> GetOnUpdateBehaviours(Int32 componentId) {
		HashSet<Type> behaviours;
	...
	private static HashSet<Type> GetOnRemoveBehaviours(Int32 componentId) {
		HashSet<Type> behaviours;

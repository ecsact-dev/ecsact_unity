...
public static class Util {
	...
	public static bool IsComponent(Type componentType) {
	...
	public static bool IsSystem(Type systemType) {
	...
	public static bool IsAction(Type actionType) {
	...
	public static object PtrToComponent<C>(IntPtr componentPtr)
	...
	public static object HandlePtrToComponent(ref object? componentObject) {
	...
	public static Type? GetComponentType(Int32 componentId) {
	...
	public static Int32 GetComponentID<T>()
		where             T : Ecsact.Component {
    return GetComponentID(typeof(T));
	}

	public static Int32 GetComponentID(Type componentType) {
		if(!IsComponent(componentType)) {
			throw new ArgumentException("Invalid component type");
		}

		var idField =
			componentType.GetField("id", BindingFlags.Static | BindingFlags.Public);
	...
	public static Int32 GetActionID<T>()
		where             T : Ecsact.Action {
    return GetActionID(typeof(T));
	}

	public static Int32 GetActionID(Type actionType) {
		return GetSystemID(actionType);
	}

	public static Int32 GetSystemID<T>()
		where             T : Ecsact.System {
    return GetSystemID(typeof(T));
	}

	public static Int32 GetSystemID(Type actionType) {
		var idField =
			actionType.GetField("id", BindingFlags.Static | BindingFlags.Public);
	...
	public static IEnumerable<ComponentIdsList> GetComponentIdPermutations(
		...
		for(; count > 0; --count) {
			...
			while(ptrs[0] <= componentIdsList.Count - count) {
				var permutationList =
					new ComponentIdsList(ptrs.Select(p => componentIdsList[p]));
